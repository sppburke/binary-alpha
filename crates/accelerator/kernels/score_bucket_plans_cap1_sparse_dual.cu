        ...
        extern "C" __global__
        void score_bucket_plans_cap1_sparse_dual(
            ...
            const int driver_key = candidate_driver_key[candidate_index];
            long long active_due = -9223372036854775807LL;
            long long total = 0;
            long long ties = 0;
            long long invalid = 0;

            long long buy_wins = 0, buy_losses = 0, buy_equity = 0, buy_peak = 0, buy_max_drawdown = 0;
            long long buy_max_drawdown_trades = 0, buy_trade_square_sum = 0;
            double buy_drawdown_square_sum = 0.0;
            long long buy_downside_square_sum = 0, buy_underwater_start_trade = -1, buy_underwater_start_ms = -1;
            long long buy_peak_trade_index = 0, buy_peak_decision_ms = -1, buy_final_settlement_ms = -1, buy_valid_trade_index = 0;
            long long buy_longest_underwater = 0, buy_longest_underwater_ms = 0, buy_current_losses = 0;
            long long buy_max_losses = 0, buy_max_drawdown_ms = 0;
            long long buy_rolling[100], buy_rolling20 = 0, buy_rolling50 = 0, buy_rolling100 = 0;
            long long buy_worst20 = 9223372036854775807LL, buy_worst50 = 9223372036854775807LL, buy_worst100 = 9223372036854775807LL;

            long long sell_wins = 0, sell_losses = 0, sell_equity = 0, sell_peak = 0, sell_max_drawdown = 0;
            long long sell_max_drawdown_trades = 0, sell_trade_square_sum = 0;
            double sell_drawdown_square_sum = 0.0;
            long long sell_downside_square_sum = 0, sell_underwater_start_trade = -1, sell_underwater_start_ms = -1;
            long long sell_peak_trade_index = 0, sell_peak_decision_ms = -1, sell_final_settlement_ms = -1, sell_valid_trade_index = 0;
            long long sell_longest_underwater = 0, sell_longest_underwater_ms = 0, sell_current_losses = 0;
            long long sell_max_losses = 0, sell_max_drawdown_ms = 0;
            long long sell_rolling[100], sell_rolling20 = 0, sell_rolling50 = 0, sell_rolling100 = 0;
            long long sell_worst20 = 9223372036854775807LL, sell_worst50 = 9223372036854775807LL, sell_worst100 = 9223372036854775807LL;
            ...
            int end = 0;
            if (driver_key >= 0) {
                start = key_chrono_offsets[driver_key];
                end = key_chrono_offsets[driver_key + 1];
            }
            for (int position = start; position < end; ++position) {
                const int row_index = key_chrono_rows[position];
                const unsigned char split_scope = split_mask[row_index];
                if (split_scope == 0) continue;
                if (feature_codes[((long long)f1) * row_count + row_index] != b1) continue;
                if (f2 >= 0 && feature_codes[((long long)f2) * row_count + row_index] != b2) continue;
                if (f3 >= 0 && feature_codes[((long long)f3) * row_count + row_index] != b3) continue;
                if (f4 >= 0 && feature_codes[((long long)f4) * row_count + row_index] != b4) continue;
                ...
                if (valid[row_index] == 0) {
                    ...
                    if (buy_win[row_index] != 0) {
                        buy_wins += 1; sell_losses += 1;
                        buy_trade_units = payout_basis; sell_trade_units = -100LL;
                    } else if (sell_win[row_index] != 0) {
                        sell_wins += 1; buy_losses += 1;
                        sell_trade_units = payout_basis; buy_trade_units = -100LL;
                    }
                }
                ...
                if (valid[row_index] != 0) {
                    ...
                    if (buy_valid_trade_index >= 50LL) buy_rolling50 -= buy_rolling[(int)((buy_valid_trade_index - 50LL) % 100LL)];
                    if (buy_valid_trade_index >= 20LL) buy_rolling20 -= buy_rolling[(int)((buy_valid_trade_index - 20LL) % 100LL)];
                    ...
                    if (buy_equity >= buy_peak) {
                        if (buy_underwater_start_trade >= 0) {
                            ...
                            if (drawdown_ms > buy_max_drawdown_ms || (drawdown_ms == buy_max_drawdown_ms && drawdown_length > buy_max_drawdown_trades)) {
                                buy_max_drawdown_ms = drawdown_ms; buy_max_drawdown_trades = drawdown_length;
                            }
                        ...
                        buy_peak_trade_index = buy_valid_trade_index; buy_peak_decision_ms = settlement_ms;
                        buy_underwater_start_trade = -1; buy_underwater_start_ms = -1;
                    }
                    ...
                    if (drawdown > 0) {
                        if (buy_underwater_start_trade < 0) {
                            buy_underwater_start_trade = buy_valid_trade_index; buy_underwater_start_ms = settlement_ms;
                        }
                    }
                    ...
                    if (sell_valid_trade_index >= 50LL) sell_rolling50 -= sell_rolling[(int)((sell_valid_trade_index - 50LL) % 100LL)];
                    if (sell_valid_trade_index >= 20LL) sell_rolling20 -= sell_rolling[(int)((sell_valid_trade_index - 20LL) % 100LL)];
                    ...
                    if (sell_equity >= sell_peak) {
                        if (sell_underwater_start_trade >= 0) {
                            ...
                            if (drawdown_ms > sell_max_drawdown_ms || (drawdown_ms == sell_max_drawdown_ms && drawdown_length > sell_max_drawdown_trades)) {
                                sell_max_drawdown_ms = drawdown_ms; sell_max_drawdown_trades = drawdown_length;
                            }
                        ...
                        sell_peak_trade_index = sell_valid_trade_index; sell_peak_decision_ms = settlement_ms;
                        sell_underwater_start_trade = -1; sell_underwater_start_ms = -1;
                    }
                    ...
                    if (drawdown > 0) {
                        if (sell_underwater_start_trade < 0) {
                            sell_underwater_start_trade = sell_valid_trade_index; sell_underwater_start_ms = settlement_ms;
                        }
                    }
                }
            ...
            buy_output[out + 0] = total; buy_output[out + 1] = buy_wins; buy_output[out + 2] = buy_losses;
            buy_output[out + 3] = ties; buy_output[out + 4] = invalid; buy_output[out + 5] = total; buy_output[out + 6] = 0;
            buy_output[out + 7] = buy_wins * payout_basis - buy_losses * 100LL; buy_output[out + 8] = buy_max_drawdown;
            buy_output[out + 9] = buy_max_losses; buy_output[out + 10] = __double_as_longlong(buy_drawdown_square_sum); buy_output[out + 11] = buy_longest_underwater;
            buy_output[out + 12] = buy_max_drawdown_trades; buy_output[out + 13] = buy_worst20 == 9223372036854775807LL ? 0 : buy_worst20;
            buy_output[out + 14] = buy_worst50 == 9223372036854775807LL ? 0 : buy_worst50; buy_output[out + 15] = buy_worst100 == 9223372036854775807LL ? 0 : buy_worst100;
            buy_output[out + 16] = buy_valid_trade_index; buy_output[out + 17] = buy_trade_square_sum; buy_output[out + 18] = buy_downside_square_sum;
            buy_output[out + 19] = buy_longest_underwater_ms; buy_output[out + 20] = buy_max_drawdown_ms;

            sell_output[out + 0] = total; sell_output[out + 1] = sell_wins; sell_output[out + 2] = sell_losses;
            sell_output[out + 3] = ties; sell_output[out + 4] = invalid; sell_output[out + 5] = 0; sell_output[out + 6] = total;
            sell_output[out + 7] = sell_wins * payout_basis - sell_losses * 100LL; sell_output[out + 8] = sell_max_drawdown;
            sell_output[out + 9] = sell_max_losses; sell_output[out + 10] = __double_as_longlong(sell_drawdown_square_sum); sell_output[out + 11] = sell_longest_underwater;
            sell_output[out + 12] = sell_max_drawdown_trades; sell_output[out + 13] = sell_worst20 == 9223372036854775807LL ? 0 : sell_worst20;
            sell_output[out + 14] = sell_worst50 == 9223372036854775807LL ? 0 : sell_worst50; sell_output[out + 15] = sell_worst100 == 9223372036854775807LL ? 0 : sell_worst100;
            sell_output[out + 16] = sell_valid_trade_index; sell_output[out + 17] = sell_trade_square_sum; sell_output[out + 18] = sell_downside_square_sum;
            sell_output[out + 19] = sell_longest_underwater_ms; sell_output[out + 20] = sell_max_drawdown_ms;
        }
