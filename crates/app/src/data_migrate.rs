...
fn observations(
    ...
    for day in days.values_mut() {
        ...
        if grid && day.full_bar_grid(newest.native_granularity)? {
...
fn convert(
    ...
    let grid = !newest.native_granularity.is_tick()
        && (predecessor_grid || newest.source_kind == SourceKind::BarParquet);
    ...
    let cov = lineage::migration_coverage(&mut manifest, history_claims, &identity, grid)?;
    manifest
        .objects
