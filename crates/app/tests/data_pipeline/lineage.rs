...
pub(super) fn legacy_import(config: &Path) -> Result<String, String> {
    let core =
        match binary_alpha_engine::config::Config::parse(&fs::read_to_string(config).unwrap()) {
            Ok(core) => core,
            Err(_) => return run(&["data", "import", "--config", config.to_str().unwrap()]),
        };
    ...
    for line in report.lines().filter(|l| l.starts_with("published ")) {
        ...
        let temporary_daily_objects = m.objects.clone();
        let lineage = m
            .objects
            .iter()
        ...
        // This helper constructs a v1 fixture; do not leave unowned objects from its
        // temporary native-v2 import for retirement to mistake for operator data.
        let retained: BTreeSet<_> = Store::filesystem(&root)
            .list_manifests()
            .unwrap()
            .into_iter()
            .flat_map(|id| {
                let value = read_json(&root.join(ds::manifest_key(&id)));
                value["objects"]
                    .as_array()
                    .unwrap()
                    .iter()
            ...
            .collect();
        for object in temporary_daily_objects {
            if !existing.contains(&object.key)
                && !retained.contains(&object.key)
                && root.join(&object.key).exists()
            {
                fs::remove_file(root.join(&object.key)).unwrap();
            }
        }
...
fn keys(m: &GenerationManifest, prefix: &str) -> BTreeMap<String, String> {
    m.objects
        .iter()
        .filter(|o| {
            o.path.starts_with(prefix)
                && !m
                    .day_inventory
                    .iter()
                    .any(|d| d.object.as_ref() == Some(&o.key) && d.rows == 0)
        })
...
fn daily_updates_preserve_days_occurrences_resume_and_reclaim_for_both_brokers() {
    for pocket in [false, true] {
        ...
        common::daily::write_coverage(&scratch, &mut pair.v2);
        if !pocket {
            ...
            let object = pair
                ...
                .unwrap();
            let mut typed = read_json(&root.join(&object.key));
            typed["acquisitions"].as_array_mut().unwrap().push(json!({
                "acquisition_id":"migrated-acquisition", "source_identity":source,
                ...
                "unresolved":[{"start":"2026-09-17T00:00:00Z","end":"2026-09-18T00:00:00Z"}, {"start":time_text(seed_ticks.last().unwrap().event_time_micros + 1), "end":"2026-09-19T00:00:00Z"}],
                "shortfalls":[{"reason":"historical_gap","unresolved":{"start":"2026-09-17T00:00:00Z","end":"2026-09-18T00:00:00Z"}}]
            }));
            fs::write(&proof, typed.to_string()).unwrap();
            pair.v2
                .objects
            ...
            fs::write(&proof, json!({"schema_version":1,"parent_generation":pair.v1.generation,"replaced_generations":[pair.v1.generation],"continuation":{"acquisition_id":"migrated-acquisition","seed":{"generation":pair.v1.generation,"source_identity":source}}}).to_string()).unwrap();
            pair.v2.objects.push(common::daily::object(
                &root,
        ...
        if !pocket {
            ...
            assert_eq!(
                coverage["acquisitions"]
                    ...
                    .find(|a| a["acquisition_id"]
                        ...
                        )["continuation"]["acquisition_id"])
                    .unwrap()["verified"][0]["start"],
            ...
            assert_eq!(
                coverage["acquisitions"]
                    ...
                    .unwrap()
                    .iter()
                    .find(|a| a["acquisition_id"] == "migrated-acquisition")
                    .unwrap()["shortfalls"][0]["reason"],
                "historical_gap"
            );
            assert!(
        ...
        for day in &mut whole.day_inventory {
            if day.family == DayFamily::Observations {
                ...
                if day.state == ds::DayState::EmptyKnown
                    && let Some(key) = day.object.take()
                {
                    whole.objects.retain(|o| o.key != key);
                }
            }
        }
        ...
        common::daily::write_coverage(&scratch, &mut whole);
    ...
    for (job, instrument) in [
        ...
        if job == "pocket" {
            ...
            let clipped = m
                ...
                .unwrap();
            assert_eq!(clipped.state, ds::DayState::Unknown);
