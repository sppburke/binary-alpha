...
fn restored_daily_seed_updates_and_archives_without_legacy_data() {
    ...
    assert_eq!(
        coverage(&root, &updated).seed.unwrap().generation,
        pair.v2.generation
    );
    assert!(!root.join(pair.v1.key()).exists());
...
fn archive_rejects_trashed_completed_legacy_object() {
    ...
    assert_eq!(
        registry_state(&scratch.path("producer/pipeline_state/registry"))["files"][&object.key]["done"],
        false
    );
}
...
fn newest_daily_skips_unrelated_declared_holdout() {
    ...
    let result = binary_alpha_app::registry::newest_daily(
    ...
    let denied = binary_alpha_app::registry::newest_daily(
        &store::Store::filesystem(&root),
        "deriv:R_100",
...
fn rebuild_checksumless_uses_one_full_readback() {
    ...
    assert_eq!(
        ...
        json!({"file_id":id,"session":null,"done":true,"bytes":identity.bytes,"sha256":identity.sha256})
    );
}
